...
const offerings = [
  ...
  },
  {
    ...
    link: "/documents/SocialMediaAnalytics.pdf",
    requiresPopup: true,
    pdfName: "Social Media Analytics - Amlgo Labs Solution",
  },
];
...
const AutomotiveOfferings = () => {
  ...
  const handleLearnMoreClick = (offering) => {
    if (offering.requiresPopup && offering.pdfName) {
      openPopup(offering.pdfName); // Trigger popup with pdfName
    }
  };
      ...
      <div className={styles.scrollDiv}>
        ...
        <div className={`${styles.offeringContent} ${fade ? styles.fadeIn : styles.fadeOut}`}>
          <div className={styles.offeringText}>
            ...
            {offerings[currentIndex].requiresPopup ? (
              <button
                className="globalButtonBlack"
                onClick={() => handleLearnMoreClick(offerings[currentIndex])}
              >
                Learn More
              </button>
            ) : (
              <Link href={offerings[currentIndex].link} target="_blank" rel="noopener noreferrer">
